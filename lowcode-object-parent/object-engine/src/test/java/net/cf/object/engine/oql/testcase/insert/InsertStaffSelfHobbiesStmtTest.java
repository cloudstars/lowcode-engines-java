...
import net.cf.object.engine.oql.testcase.Staff;
...
public class InsertStaffSelfHobbiesStmtTest extends AbstractOqlTest implements InsertStaffSelfHobbiesTest {

    public InsertStaffSelfHobbiesStmtTest() {
        super(OQL_FILE_PATH);
    }
    ...
    public void testInsertStaff() {
        OqlInfo oqlInfo = this.oqlInfos.get(OQL_INSERT_STAFF);
        assert (oqlInfo != null && oqlInfo.oql != null && oqlInfo.sql != null);

        // 断言解析出一条OQL语句，并且OQL语句输出OQL文本是符合预期的
        XObject object = ObjectTestResolver.resolveObject(Staff.NAME);
        ...
        assert (osExpr instanceof SqlIdentifierExpr && Staff.NAME.equals(((SqlIdentifierExpr) osExpr).getName()));
