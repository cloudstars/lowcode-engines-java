...
public abstract class AbstractSelectWhereInRepoTest
        extends AbstractOqlRepoTest
        implements SelectWhereInTest {

    protected AbstractSelectWhereInRepoTest() {
        super(OQL_FILE_PATH);
    }
    ...
    public void testSelectWhereIn() {
        OqlInfo oqlInfo = this.oqlInfos.get(OQL_SELECT_IN);
    ...
    public void testSelectWhereNotIn() {
        OqlInfo oqlInfo = this.oqlInfos.get(OQL_SELECT_NOT_IN);
        OqlSelectStatement oqlStmt = OqlStatementUtils.parseSingleSelectStatement(this.resolver, oqlInfo.oql);
        List<Map<String, Object>> resultList = this.engine.queryList(oqlStmt, new HashMap<>());
        assert (resultList.size() == 1);
        assert (DataCompareTestUtils.equalsList(resultList, oqlInfo.resultMaps));
