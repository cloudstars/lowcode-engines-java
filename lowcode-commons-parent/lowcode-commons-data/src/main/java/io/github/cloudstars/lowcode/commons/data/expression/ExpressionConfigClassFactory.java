...
public class ExpressionConfigClassFactory {
    ...
    public static void register(Class<? extends XExpressionConfig> configClass) {
        ...
        if (annos.length == 0) {
            throw new RuntimeException("表达式" + typeName + "必须添加注解@ExpressionConfigClass，注册失败！");
        }

        try {
            configClass.getConstructor(JsonObject.class);
        } catch (Exception e) {
            throw new RuntimeException("表达式" + typeName + "必须有一个带JsonObject参数的public构造函数！");
        }
    ...
    public static Class<? extends XExpressionConfig> get(String typeName) {
        Class<? extends XExpressionConfig> typeClass = configClassMap.get(typeName);
        if (typeClass == null) {
            throw new RuntimeException("表达式" + typeName + "不存在！");
        }
