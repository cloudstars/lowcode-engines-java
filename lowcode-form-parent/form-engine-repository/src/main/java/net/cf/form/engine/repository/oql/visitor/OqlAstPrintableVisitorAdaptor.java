...
import net.cf.form.engine.repository.oql.FastOqlException;
...
public class OqlAstPrintableVisitorAdaptor extends OqlAstVisitorAdaptor implements OqlAstPrintableVisitor {
    ...
    protected final void printMethod(OqlMethodInvokeExpr x) {
        ...
        this.print('(');
        List<QqlExpr> args = x.getArguments();
        if (args.size() > 0) {
            this.outputByCommaSeparated(args, (arg) -> {
                this.printExpr((QqlExpr) arg);
            });
        }
        this.print(')');
    }
     ...
     * @param x
     */
    protected final void printExpr(QqlExpr x) {
