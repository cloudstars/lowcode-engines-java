...
@SpringBootTest
@ActiveProfiles("h2")
public class UpdateMainDetailDataTest implements UpdateMainAndDetailTest {
    ...
    public void testUpdateTravelAndSchedules() {
        OqlUpdateStatement statement = OqlUtils.parseSingleUpdateStatement(UPDATE_TRAVEL_AND_SCHEDULES_OQL);
        this.driver.update(statement);
        this.checkUpdateTravelAndSchedulesResult();
    }

    @Test
    @Override
    public void testUpdateTravelAndSchedulesVar() {
        OqlUpdateStatement statement = OqlUtils.parseSingleUpdateStatement(UPDATE_TRAVEL_AND_SCHEDULES_VAR_OQL);
        Map<String, Object> paramMap = JsonUtils.loadMapFromClasspath("data/testUpdateTravelSchedules.json");
     ...
     * 检查更新后的结果
     */
    private void checkUpdateTravelAndSchedulesResult() {
        // 更新后查询数据数被更新的记录
        ...
        Assert.assertTrue("2023-06-07".equals(DateUtils.formatDate((Date) startDateValue)));
        Object endDateValue = resultMap.get("endDate");
        Assert.assertTrue(endDateValue instanceof Date);
        Assert.assertTrue("2023-06-10".equals(DateUtils.formatDate((Date) endDateValue)));
    }
}
