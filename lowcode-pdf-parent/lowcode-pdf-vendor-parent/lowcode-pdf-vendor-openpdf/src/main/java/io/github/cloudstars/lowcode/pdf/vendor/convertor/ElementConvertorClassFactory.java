...
public class ElementConvertorClassFactory {
    ...
    public static void register(Class<? extends XElementConvertor> convertorClass) {
        ...
        ElementConvertorClass[] annotations = convertorClass.getAnnotationsByType(ElementConvertorClass.class);
        if (annotations.length == 0) {
            throw new RuntimeException("OpenPdf元素转换器类型[" + typeName + "]必须添加注解@NodeConfigClass，注册失败！");
        }

        try {
            convertorClass.getConstructor(JsonObject.class);
        } catch (Exception e) {
            throw new RuntimeException("OpenPdf元素转换器类型" + typeName + "必须有一个带JsonObject参数的public构造函数！");
        }
